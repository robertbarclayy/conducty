...
async function validatePluginManifest(pluginDir, pluginName, pluginManifest, expectedName) {
  ...
  if (expectedName && pluginManifest.name && pluginManifest.name !== expectedName) {
    addError(`${pluginName}: manifest name does not match expected plugin name ("${pluginManifest.name}").`);
  }

  if (typeof pluginManifest.displayName !== "string" || pluginManifest.displayName.length === 0) {
    addError(`${pluginName}: "displayName" is required in plugin.json.`);
  }

  if (typeof pluginManifest.description !== "string" || pluginManifest.description.length === 0) {
    addError(`${pluginName}: "description" is required in plugin.json.`);
  }

  if (typeof pluginManifest.version !== "string" || pluginManifest.version.length === 0) {
    addError(`${pluginName}: "version" is required in plugin.json.`);
  }
  ...
  if (typeof pluginManifest.license !== "string" || pluginManifest.license.length === 0) {
    addError(`${pluginName}: "license" is required in plugin.json.`);
  }
...
async function main() {
  ...
  const rootPluginManifestPath = path.join(repoRoot, ".cursor-plugin", "plugin.json");
  const hasMarketplace = await pathExists(marketplacePath);
  const hasRootPlugin = await pathExists(rootPluginManifestPath);

  if (hasMarketplace && hasRootPlugin) {
    addError("Found both .cursor-plugin/marketplace.json and .cursor-plugin/plugin.json. Keep only one packaging mode.");
    summarizeAndExit();
    return;
  }

  if (hasRootPlugin) {
    const pluginManifest = await readJsonFile(rootPluginManifestPath, "Root plugin manifest");
    if (!pluginManifest) {
      summarizeAndExit();
      return;
    }

    await validatePluginManifest(repoRoot, pluginManifest.name || "root-plugin", pluginManifest);
    summarizeAndExit();
    return;
  }

  if (!hasMarketplace) {
    addError("Missing plugin manifest. Expected .cursor-plugin/plugin.json for single-plugin layout.");
    summarizeAndExit();
    return;
  }
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    await validatePluginManifest(pluginDir, entry.name, pluginManifest, entry.name);
  }
