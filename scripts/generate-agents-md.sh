...
Conducty skills are installed alongside this file. Use your tool's native skill discovery to list and load them (e.g. `skill` tool in OpenCode, native discovery in Codex, or ask "list conducty skills").
...
**Tool Mapping:** When skills reference Cursor-specific tools, substitute your platform's equivalents:
- `TodoWrite` → your native todo/task tool
- `Task` tool with subagents → your native subagent system
- `Read`, `Write`, `Edit` → your native file tools
- `Shell` / `Bash` → your native command execution tool
